...
public class FragmentCursor {
    ...
    public FragmentCursor(Fragment fragment, int position) {
        Helper.abortIf(fragment == null, "NullPointerException: you have to give a fragment, not null");
        Helper.abortIf(position < 0 || fragment.length() < position, "position is out of fragment bounds");
    ...
    /**
     * Return True if the position is between 0 and Fragment length, abort and false otherwise.
     *
     * @param position The position of the character
     * @return True if the position is between 0 and Fragment length, abort and false otherwise.
     */
    public boolean checkPosition(int position) {
        Helper.abortIf(!(position >= 0 && position <= this.fragment.length()), "position is out of string bounds");
        return true;
    }
    ...
    public Fragment cutOff() {
        Helper.abortIf(!checkPosition(this.position - 1) || !checkPosition(this.position + 1),
                "the current position is at the beginning or end of the current fragment");

        Fragment result = new Fragment(new Style(this.fragment.getStyle().isBold(),
                this.fragment.getStyle().isItalic()), this.fragment.substring(this.position,
                this.fragment.length()));
        this.fragment.deleteSubstring(this.position, this.fragment.length());
        return result;
    }
}
